    ...
    while not finished:
        ...
        if cheapestState is None:
            ...
            for direction in [(1,0), (-1, 0), (0,1), (0,-1)]:
                ...
                if 0 <= nextPos[0] <= maxLen and 0 <= nextPos[1] <= maxLen and nextPos not in seenStates and nextPos not in funkObstructions:
...
while lowSize+1 < MaxSize:
    middleSize = (lowSize+MaxSize)//2
    middleObstructions = LowObstructions.copy()
    for newInstruction in range(lowSize, middleSize):
        middleObstructions.add(data[newInstruction])
    path = checkVal(middleObstructions)
    if path:
        lowSize = middleSize
        LowObstructions = middleObstructions
    else:
        MaxSize = middleSize

print("data is size", len(data), "broke on", MaxSize, "with min size", lowSize)
print(data[MaxSize-1])
print(checkVal)


# noPath = False
# finalObstruction = None
# obstructions = set()
# for byteFall in range(0, len(data)):

#     if noPath:
#         break


#print(finalObstruction)
